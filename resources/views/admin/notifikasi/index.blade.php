...
<script>
            ...
            try {
                ...
                if (!response.ok) {
                    return Swal.fire({
                        ...
                        data.approvedQty = response.jumlah_lama;
                    });
                }
    ...
    // Simpan jumlah lama sebelum user mengubah
    const oldQty = data.approvedQty;
    ...
    fetch(url, {
        ...
        body: JSON.stringify({
            jumlah_disetujui: data.approvedQty
        })
    })
    .then(async res => {
        const response = await res.json();

        // ❌ Jika gagal (jumlah > stok)
        if (!res.ok) {
            Swal.fire({
                icon: 'error',
                title: 'Gagal!',
                ...
                // Kembalikan input ke jumlah lama
                data.approvedQty = response.jumlah_lama ?? oldQty;
            });
